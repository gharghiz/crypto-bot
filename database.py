...
def init_db():
    try:
        if USE_POSTGRES:
            ...
            cur.execute("""CREATE TABLE IF NOT EXISTS posted_news (
            ...
            for col in ["summary","sentiment","reason"]:
                try: cur.execute(f"ALTER TABLE posted_news ADD COLUMN {col} TEXT")
                except: pass
            conn.commit(); cur.close(); conn.close()
        else:
            with get_sqlite_conn() as conn:
                conn.execute("""CREATE TABLE IF NOT EXISTS posted_news (
                    id TEXT PRIMARY KEY, title TEXT, source TEXT, posted_at TEXT,
                    summary TEXT, sentiment TEXT, reason TEXT)""")
                for col in ["summary","sentiment","reason"]:
                    try: conn.execute(f"ALTER TABLE posted_news ADD COLUMN {col} TEXT")
                    except: pass
                conn.commit()
...
def is_posted(news_id: str) -> bool:
    try:
        ...
        logger.error(f"❌ is_posted: {e}"); return False
...
def mark_posted(news_id: str, title: str, source: str,
    ...
    try:
        if USE_POSTGRES:
            conn = get_pg_conn(); cur = conn.cursor()
            cur.execute("""INSERT INTO posted_news (id,title,source,posted_at,summary,sentiment,reason)
                VALUES (%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (id) DO NOTHING""",
                (news_id,title,source,posted_at,summary,sentiment,reason))
            conn.commit(); cur.close(); conn.close()
        else:
            with get_sqlite_conn() as conn:
                conn.execute("""INSERT OR IGNORE INTO posted_news
                    (id,title,source,posted_at,summary,sentiment,reason) VALUES (?,?,?,?,?,?,?)""",
                    (news_id,title,source,posted_at,summary,sentiment,reason))
                conn.commit()
        cache_clear()
    except Exception as e:
        ...
        if USE_POSTGRES:
            ...
            with get_sqlite_conn() as conn:
                return [r["title"] for r in conn.execute(
                    "SELECT title FROM posted_news ORDER BY posted_at DESC LIMIT ?", (limit,)).fetchall()]
    except Exception as e:
        logger.error(f"❌ get_recent_titles: {e}"); return []
...
def get_news_by_id(news_id: str):
    try:
        ...
        logger.error(f"❌ get_news_by_id: {e}"); return None
...
def get_stats():
    ...
    try:
        ...
        cache_set("stats", result); return result
    except Exception as e:
        logger.error(f"❌ get_stats: {e}"); return {"total":0,"today":0,"sources":[]}

def cleanup_old(hours: int = 6):
    """حذف الأخبار بعد X ساعات باش البوت ينشرها من جديد"""
    try:
        if USE_POSTGRES:
            conn = get_pg_conn(); cur = conn.cursor()
            # نستعملو comparison مع text مباشرة — أسهل وأضمن
            cur.execute("""
            ...
            with get_sqlite_conn() as conn:
                ...
                conn.commit()
        logger.info(f"🗑️ حذفنا {deleted} خبر قديم (>{hours}h)")
        cache_clear()
    except Exception as e:
